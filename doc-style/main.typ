...
#let (
  ...
  link, emphasis, strong, code, code-block, quote, figure-caption,
  list, enum, figure, divider, footnote-entry,
) = l
...
// ─── Per-call overrides ────────────────────────────────────────────────────────
#text-1(weight: 700, fill: rgb("#222"))[Per-call: bold + dark gray.]
...
#text-1(..partial)[Bold; fill and tracking stay at the layout defaults.]
...
#text-1(justify: true)[
  ...
  via `overrides: (text-1: (justify: true))`. Useful for body text in book-like
...
This is a plain paragraph written in pure Typst markup. It picks up text-1's
...
text-3 (one step below body) and uses the muted foreground color, with
